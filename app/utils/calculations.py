...
def calculate_drawdown(trades: List[Trade]) -> Dict[str, float]:
    max_drawdown = 0
    peak = float('-inf')
    for trade in trades:
        current_value = trade.exit_price * trade.position_size if trade.is_closed else trade.entry_price * trade.position_size
        peak = max(peak, current_value)
        drawdown = (peak - current_value) / peak if peak > 0 else 0
        max_drawdown = max(max_drawdown, drawdown)
    return {"max_drawdown": max_drawdown * 100}
